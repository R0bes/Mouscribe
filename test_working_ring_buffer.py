#!/usr/bin/env python3
"""
Einfache funktionierende Ring Buffer Lösung
- Verwendet sd.rec() in einem Loop (bewährt)
- Einfacher Ring Buffer mit deque
...
class WorkingRingBufferAgent:
    """Agent mit funktionierendem Ring Buffer."""
    ...
    def start(self):
        """Starte den Agent mit funktionierendem Ring Buffer."""
        if self.running:
            logger.warning("⚠️ Agent läuft bereits")
            return

        logger.info("🚀 Starte Agent mit funktionierendem Ring Buffer...")
        ...
        logger.info("✅ Agent mit funktionierendem Ring Buffer läuft")
    ...
    def _audio_loop(self):
        """Audio Loop mit funktionierendem Ring Buffer."""
                ...
                if audio_data is not None and len(audio_data) > 0:
                    ...
                    with self.ring_buffer_lock:
                        ...
                        if self.chunk_counter % 5 == 0:  # Alle 5 Chunks
                            logger.info(f"📊 Chunk #{self.chunk_counter}: Ring Buffer gefüllt mit {len(self.ring_buffer)} Samples (max: {self.ring_buffer.maxlen})")
...
def main():
    """Hauptfunktion für Agent mit funktionierendem Ring Buffer."""
    print("🧪 Agent mit funktionierendem Ring Buffer Test")
    print("=" * 60)
    ...
    # Starte Agent
    print("\n🚀 Starte Agent mit funktionierendem Ring Buffer...")
    agent = WorkingRingBufferAgent()
    agent.start()
    
    print("\n💡 Agent mit funktionierendem Ring Buffer läuft!")
