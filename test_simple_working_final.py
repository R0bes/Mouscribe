#!/usr/bin/env python3
"""
Einfache funktionierende Lösung ohne komplexe Ring Buffer
- Verwendet sd.rec() in einem Loop (bewährt)
- Einfacher Audio Buffer
...
class SimpleWorkingAgent:
    """Agent mit einfacher funktionierender Lösung."""
    ...
    def __init__(self):
        ...
        # Einfacher Audio Buffer (kein Ring Buffer)
        self.audio_buffer = []
    ...
    def start(self):
        """Starte den Agent mit einfacher Lösung."""
        if self.running:
            logger.warning("⚠️ Agent läuft bereits")
            return

        logger.info("🚀 Starte Agent mit einfacher Lösung...")
        ...
        # Starte Audio Loop Thread
        self.audio_thread = threading.Thread(target=self._audio_loop, daemon=True)
        self.audio_thread.start()
        
        logger.info("✅ Agent mit einfacher Lösung läuft")
    ...
    def _audio_loop(self):
        """Audio Loop mit einfacher Lösung."""
        logger.info("🔄 Audio Loop gestartet")
        
        last_analysis_time = 0
        analysis_interval = 0.5  # Alle 0.5 Sekunden analysieren
            ...
            try:
                ...
                if audio_data is not None and len(audio_data) > 0:
                    ...
                    with self.buffer_lock:
                        ...
                        # Debug: Zeige Buffer Status
                        if self.chunk_counter % 5 == 0:  # Alle 5 Chunks
                            logger.info(f"📊 Chunk #{self.chunk_counter}: Buffer gefüllt mit {len(self.audio_buffer)} Samples")
                    ...
                    if current_time - last_analysis_time >= analysis_interval:
                        ...
                        with self.buffer_lock:
                            if len(self.audio_buffer) >= self.analysis_samples:
                                self._analyze_current_segment()
                            else:
                                logger.info(f"⚠️ Noch nicht genug Audio für Analyse: {len(self.audio_buffer)}/{self.analysis_samples}")
    ...
    def _analyze_current_segment(self):
        ...
        try:
            ...
            # Hole aktuelles Segment aus Buffer
            with self.buffer_lock:
                # Hole die letzten analysis_samples aus dem Buffer
                current_segment = np.array(self.audio_buffer[-self.analysis_samples:])
...
def main():
    """Hauptfunktion für Agent mit einfacher Lösung."""
    print("🧪 Agent mit einfacher Lösung Test")
    print("=" * 60)
    ...
    # Starte Agent
    print("\n🚀 Starte Agent mit einfacher Lösung...")
    agent = SimpleWorkingAgent()
    agent.start()
    
    print("\n💡 Agent mit einfacher Lösung läuft!")
