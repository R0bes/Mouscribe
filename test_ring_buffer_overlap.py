#!/usr/bin/env python3
"""
Echter Ring Buffer mit überlappenden Segmenten
- Ring Buffer für kontinuierliche Audio-Daten
- Echte überlappende Segmente mit Sliding Window
...
class RingBufferOverlapAgent:
    """Agent mit echtem Ring Buffer und überlappenden Segmenten."""
    ...
    def __init__(self):
        ...
        # Echter Ring Buffer für überlappende Segmente
        ...
        # Ring Buffer: Speichert 6 Sekunden Audio (2x Segment-Dauer für Überlappung)
        self.ring_buffer_size = self.segment_samples * 2
    ...
    def start(self):
        """Starte den Agent mit echtem Ring Buffer."""
        if self.running:
            logger.warning("⚠️ Agent läuft bereits")
            return

        logger.info("🚀 Starte Agent mit echtem Ring Buffer...")
        ...
        logger.info("✅ Agent mit echtem Ring Buffer läuft")
    ...
    def _ring_buffer_audio_loop(self):
        """Audio-Loop mit echtem Ring Buffer."""
        logger.info("🔄 Audio-Loop mit echtem Ring Buffer gestartet")
        ...
        while self.running:
            try:
                ...
                if audio_data is not None and len(audio_data) > 0:
                    ...
                    if self.ring_buffer_pos >= self.segment_samples:
                        # Erstelle überlappendes Segment aus Ring Buffer
    ...
    def _get_overlapping_segment(self) -> np.ndarray:
        """Hole überlappendes Segment aus Ring Buffer."""
        with self.ring_buffer_lock:
            # Hole die letzten segment_samples aus dem Ring Buffer
            if self.ring_buffer_pos < self.segment_samples:
                # Noch nicht genug Daten
                return np.array([])
            
            # Berechne Start-Position für das Segment
            start_pos = (self.ring_buffer_pos - self.segment_samples) % self.ring_buffer_size
            
            # Hole das Segment
            if start_pos + self.segment_samples <= self.ring_buffer_size:
                # Segment ist nicht über die Ring Buffer Grenze
                segment = self.ring_buffer[start_pos:start_pos + self.segment_samples].copy()
            else:
                # Segment geht über die Ring Buffer Grenze
                part1 = self.ring_buffer[start_pos:].copy()
                part2 = self.ring_buffer[:self.segment_samples - len(part1)].copy()
                segment = np.concatenate([part1, part2])
            
            return segment
    
    def _process_overlapping_segment(self):
        """Verarbeite überlappendes Audio-Segment aus Ring Buffer."""
        try:
            # Hole überlappendes Segment aus Ring Buffer
            current_segment = self._get_overlapping_segment()
...
def main():
    """Hauptfunktion für Agent mit echtem Ring Buffer."""
    print("🧪 Agent mit echtem Ring Buffer Test")
    print("=" * 60)
    ...
    # Starte Agent
    print("\n🚀 Starte Agent mit echtem Ring Buffer...")
    agent = RingBufferOverlapAgent()
    agent.start()
    
    print("\n💡 Agent mit echtem Ring Buffer läuft!")
